...
class CompactCalendarController implements Serializable{
    ...
    private static final long serialVersionUID = 2475651364277455366L;
    ...
    List<Event> getCalendarEventsFor(long epochMillis) {
        ...
        if (events == null) {
            ...
            Collections.sort(events.getEvents(), new Comparator<Event>() {
                @Override
    ...
    void drawEvents(Canvas canvas, Calendar currentMonthToDrawCalender, int offset) {
        ...
        if (uniqEventses != null) {
            for (int i = 0; i < uniqEventses.size(); i++) {
                ...
                boolean isCurrentSelectedDay = (currentCalender.get(Calendar.DAY_OF_MONTH) == dayOfMonth) && currentSelectedDayBackgroundColor == Color.rgb(224, 247, 250);
    ...
    void drawMonth(Canvas canvas, Calendar monthToDrawCalender, int offset) {
        ...
        for (int dayColumn = 0, dayRow = 0; dayColumn <= 6; dayRow++) {
            ...
            if (dayRow == 0) {
                ...
                boolean shouldDrawDaysHeader = true;
